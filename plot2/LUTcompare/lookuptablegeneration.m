...
thrusparam = sparameters('C:\Users\grgo8200\repos\summer2025_loadpull_repo\data\deembedsparam0710\LPSetup_Validation_2portthru_20250707_direct 4.s2p');
...
for k = 1 : size(data.freq,2)
    ...
    filepath = strcat('../../data/LUT3/',filename,'.mat');
    if isfile(filepath)
        ...
        save(strcat('../../data/LUT3/',filename,'.mat'), 'LUT');  
    else
        ...
        [~, idx] = unique(col1);
        LUT = [ col1(idx), col2(idx)] ;
        save(strcat('../../data/LUT3/',filename,'.mat'), 'LUT','waves'); 
    end
end

classdef DataTableClass
    ...
    methods
            ...
            for lp_idx=1:size(fn,1)
                ...
                elseif fn{lp_idx} == "Configuration"
                    ...
                    try
                        if fn{lp_idx}(1) == 'L'
                            fileflag = 1;
                        elseif fn{lp_idx}(2) =='P'
                            fileflag = 2;
                        end
                        obj.data = [obj.data; obj.datapointtable(json_txt.(fn{lp_idx}),frequency,obj.get_setpower(fn{lp_idx}),fileflag)];
                    catch
                    end
                end
            end
        ...
        function obj = freqpowerbias_nofit(obj)
            [~,bins] = discretize(obj.data.GateV,length(unique(round(obj.data.GateV,2))));
            nonNanIdx = ~isnan(obj.data.SetPower);
            nonNanElements = obj.data.SetPower(nonNanIdx);
            obj.power_list = unique(nonNanElements)';
        ...
        function fpb_row = get_matcheddata(obj)
            rf = rowfilter(obj.data);
            fpb_row = obj.freqpowerbiastable(rf.GammaLoad == 0 ,:);
        end
        ...
        function tableoutput = datapointtable(obj,json_txt, frequency,SetPower,fileflag)
            try
                ...
                if isfield(json_txt,'DCParameters')
                    if isfield(json_txt.DCParameters,'gateCurrent'); GateI = json_txt.DCParameters.gateCurrent;
                    elseif isfield(json_txt.DCParameters,'gatecurrent'); GateI = json_txt.DCParameters.gateCurrent; else; GateI = nan; end;
                    if isfield(json_txt.DCParameters,'gateVoltage'); GateV = json_txt.DCParameters.gateVoltage;
                    elseif isfield(json_txt.DCParameters,'gatevoltage'); GateV = json_txt.DCParameters.gateVoltage; else; GateV = nan; end;
                    if isfield(json_txt.DCParameters,'drainCurrent'); DrainI = json_txt.DCParameters.drainCurrent;
                    elseif isfield(json_txt.DCParameters,'draincurrent'); DrainI = json_txt.DCParameters.drainCurrent; else; DrainI = nan; end;
                    if isfield(json_txt.DCParameters,'drainVoltage'); DrainV = json_txt.DCParameters.drainVoltage;
                    elseif isfield(json_txt.DCParameters,'drainvoltage'); DrainV = json_txt.DCParameters.drainVoltage; else; DrainV = nan; end;
                else
                ...
                if isfield(json_txt,'PAPerformance')
                    if isfield(json_txt.PAPerformance,'PAE'); 
                        if fileflag == 1
                            PAE = json_txt.PAPerformance.PAE; 
                        elseif fileflag == 2
                            PAE = json_txt.PAPerformance.PAE*100;
                        end
                    end
        ...
        function [ScaleFactorOffset,...
                ScaleFactorSampler1_Av,ScaleFactorSampler2_Av,...
                RMSError, GoodnessOfFit, Sampler1_phase, Sampler2_phase, GoodnessOfFitPhase, MaxError, MinError]  = samplerfitting(obj,x,v1,v2)
            ...
            RMSError = rmse(abs(x),sqrt(ScaleFactor(2)*v1+ScaleFactor(3)*v2 + ScaleFactor(4)));
            MaxError = max(max(abs(abs(x)-abs(sqrt(ScaleFactor(2)*v1+ScaleFactor(3)*v2 +ScaleFactor(4))))))
            MinError = min(min(abs(abs(x)-abs(sqrt(ScaleFactor(2)*v1+ScaleFactor(3)*v2 +...
                 ScaleFactor(4))))))
        end

        function [bL2, ScaleFactorKappa1,ScaleFactorKappa2,...
                ScaleFactorSampler1_Av,ScaleFactorSampler2_Av,...
                RMSError, MaxError, MinError, GoodnessOfFit]  = samplerfittingpower(obj,x,v1,v2,power)
            ...
            MaxError = max(max(abs(abs(x)-abs(sqrt( 1/(2*bL2*ScaleFactorSampler1_Av)*(v1-ScaleFactorKappa1)+...
                1/(2*bL2*ScaleFactorSampler2_Av)*(v2-ScaleFactorKappa2) - 1)))));
            MinError = min(min(abs(abs(x)-abs(sqrt( 1/(2*bL2*ScaleFactorSampler1_Av)*(v1-ScaleFactorKappa1)+...
                1/(2*bL2*ScaleFactorSampler2_Av)*(v2-ScaleFactorKappa2) - 1)))));
        end

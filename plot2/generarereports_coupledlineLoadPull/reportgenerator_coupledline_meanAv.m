...
coupledline_data = load('coupledline_data_newLUT.mat').coupledline_data_newLUT

filename = 'coupledline_data_new_meanAv';
...
SamplerV_Mean = nan;
pb_fit = table(ScaleFactorSampler1_Av, ...
    ScaleFactorSampler2_Av, ...
    ScaleFactorOffset, ...
    SamplerV_Mean);
...
for x = 1:size(coupledline_data.sampler_bias_list,2)-1
    for y = 1:size(coupledline_data.power_list,2)
        pb_fit =  coupledline_data.Avaveragefreq( ...
...
for i = 1:size(biaslowlist,2)-1
    ...
    saveas(gcf,strcat('../png/bias',num2str(i),'.png'));
end
...
for x = 1:size(coupledline_data.sampler_bias_list,2)-1
    for y = 1:size(coupledline_data.power_list,2)
        for z = 1:size(coupledline_data.freq_list,2)
            ...
            T = coupledline_data.gaintable.get_gain( ...
            ...
            if ~isempty(T)
                ...
                subplot(3,3,4)
                set(gca,'ColorOrderIndex',1)
                scatter(abs(gamma),abs(sqrt(T.Av1_VpermW*samp1raw ...
                ...
                saveas(gcf,strcat('../png/slide_new',num2str(x),num2str(y),num2str(z),'.png'));
                close all

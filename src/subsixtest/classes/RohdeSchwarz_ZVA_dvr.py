...
class RohdeSchwarzZVA(VectorNetworkAnalyzerCtg1):
	...
	def init_zva_subsix_loadpull(self, config_test:ZVAConfig, channel = '1', 
		...
		self.write("CALCulate1" + str(channel) + ":PARameter:SDEFine 'Trc" + str(input_awave_trace)
				+ "', 'A" + str(config_test.input_port)
				+ "D" + str(config_test.input_port) + "'")
		self.write("CALCulate" + str(channel) + ":PARameter:SDEFine 'Trc" + str(output_awave_trace) 
				+ "', 'A" + str(config_test.output_port) 
				+ "D" + str(config_test.input_port) + "'")
		self.write("CALCulate" + str(channel) + ":PARameter:SDEFine 'Trc" + str(input_bwave_trace) 
				+ "', 'B" + str(config_test.input_port) 
				+ "D" + str(config_test.input_port) + "'")
		self.write("CALCulate" + str(channel) + ":PARameter:SDEFine 'Trc" + str(output_bwave_trace) 
	...
	def get_trace_data(self, tracenumber:int, channel = '1'):
		...
		self.write("CALCulate1:PARameter:SELect " + trace_name)
		...
		return {'trace_name': trace_name ,'x': freqs_Hz, 'y_real': real_tokens, 'y_imag': imag_tokens, 'x_units': 'Hz', 'y_units': 'EWC wave data (voltage)'}    